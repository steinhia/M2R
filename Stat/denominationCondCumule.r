...
tgc <- summarySE(tab, measurevar="scoreCumule", groupvars=c("jour","condition"))
p<-ggplot(data=tgc, aes(x=jour, y=scoreCumule, fill=condition)) + 
  geom_bar(position=position_dodge(), stat="identity") +
  geom_errorbar(aes(ymin=tgc$scoreCumule-tgc$se, ymax=tgc$scoreCumule+tgc$se),
                width=.2,                    # Width of the error bars
                position=position_dodge(.9)) +
  ggtitle("Scores cumulés en dénomination")
...
tgc <- summarySE(tab, measurevar="scoreCumule", groupvars=c("jour","histoire"))
p<-ggplot(data=tgc, aes(x=jour, y=scoreCumule, fill=histoire)) + 
  geom_bar(position=position_dodge(), stat="identity") +
  geom_errorbar(aes(ymin=tgc$scoreCumule-tgc$se, ymax=tgc$scoreCumule+tgc$se),
                width=.2,                    # Width of the error bars
                position=position_dodge(.9)) +
...
tab$score4 <- as.character(tab$scoreCumule) 
...
tab$score3 <- as.character(tab$scoreCumule) 
...
mod <- gamlss(scoreCumule~jour*condition+ re(random=~1|id) ,nu.formula = ~jour*condition+ re(random=~1|id), tau.formula = ~jour*condition+ re(random=~1|id),family=BEINF,data=tab)
...
mod_choisi <- gamlss(scoreCumule~jour+condition+ re(random=~1|id) ,nu.formula = ~jour+ re(random=~1|id), tau.formula = ~jour+condition+ re(random=~1|id),family=BEINF,data=tab)
plot(mod_choisi)
Rsq(mod_choisi)
