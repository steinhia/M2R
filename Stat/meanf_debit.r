...
tgc <- summarySE(tab, measurevar="mean.f", groupvars=c("jour","condition"))
p<-ggplot(data=tgc, aes(x=jour, y=mean.f, fill=condition)) + 
...
fit0 <-  lme(mean.f~jour*condition, random=~1|id,data=tab, method="ML",na.action=na.exclude)
...
fit1c <- lme(mean.f~jour*condition, random=list(id=pdBlocked(list(pdIdent(~1),pdIdent(~condition-1)))),data=tab, method="ML",na.action=na.exclude)
fit1j <- lme(mean.f~jour*condition, random=list(id=pdBlocked(list(pdIdent(~1),pdIdent(~jour-1)))),data=tab, method="ML",na.action=na.exclude)
...
fit_varc <- lme(mean.f~jour*condition, random=~1|id,weights=varIdent(form=~1|condition),data=tab, method="ML",na.action=na.exclude)
anova(fit0,fit_varc)

fit_varj <- lme(mean.f~jour*condition, random=~1|id,weights=varIdent(form=~1|jour),data=tab, method="ML",na.action=na.exclude)
anova(fit0,fit_varj)
...
mod_choisi <- lme(mean.f~1, random=~1|id,weights=varIdent(form=~1|jour),data=tab, method="ML",na.action=na.exclude)
