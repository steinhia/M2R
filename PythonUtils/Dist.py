...
# 1 étape de calcul de la distance entre 2 mots
def _edit_dist_step(lev, i, j, s1, s2, substitution_cost=1.0/0.7):
    ...
    b = lev[i][j - 1] + 1
    # substitution
    tmp=(float(phonemeDist(NaturalClasses,c1,c2)))
    c = lev[i - 1][j - 1] + substitution_cost*tmp
    # pick the cheapest
    lev[i][j] = min(a, b, c)

# distance entre 2 mots à proprement parler
